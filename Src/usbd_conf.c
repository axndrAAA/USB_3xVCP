...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  volatile HAL_StatusTypeDef stat = HAL_OK;

  stat = HAL_PCDEx_SetRxFiFo(&hpcd_USB_OTG_HS, 0x200);
  stat = HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_HS, 0, 0x0040);

  stat = HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_HS, 1, 0x0040);
  stat = HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_HS, 2, 0x0040);

  stat = HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_HS, 3, 0x0040);
  stat = HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_HS, 4, 0x0040);
  
  stat = HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_HS, 5, 0x0040);
  stat = HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_HS, 6, 0x0040);
  stat = HAL_OK;
  }
  return USBD_OK;
}
